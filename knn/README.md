...
The next step is to run the knn backend in your machine. The application is written in Python, so you need to have `python3` installed.

In order to start the knn backend, run the following commands in a dedicated terminal:
...
This will run the knn backend and send the corresponding notices to port `5004`.
...
When you add an input, you should see it being processed by the knn backend as follows:
