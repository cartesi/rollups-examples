...
This example implements the same behavior as the [Echo DApp written in Python](../echo), but here the back-end is written in Rust. As the other example, the DApp simply copies (or "echoes") each input received as a corresponding output notice.

## Building the environment

To run the echo-rust example, clone the repository as follows:
...
First, go to a separate terminal window, switch to the `echo-rust/contracts` directory, and run `yarn`:

```shell
$ cd echo-rust/contracts/
$ yarn
```
...
```shell
$ npx hardhat --network localhost echo-rust:addInput --input "0x63617274657369"
```
...
```shell
$ npx hardhat --network localhost echo-rust:getNotices --epoch 0 
```
...
```shell
$ npx hardhat --network localhost echo-rust:addInput --input 'Hello there!'
```

To retrieve notices interpreting the payload as a UTF-8 string, you can use the `--payload string` option:

```shell
$ npx hardhat --network localhost echo-rust:getNotices --epoch 0 --payload string
...
```shell
$ npx hardhat echo-rust:addInput --help
```
...
The next step is to run the echo-rust backend in your machine. The application is written in Rust, so you need to have rust installed in order to recompile it.

In order to start the echo-rust backend in host mode, run the following commands in a dedicated terminal:

```shell
$ cd echo-rust/backend/
$ ./run-host.sh
```

This will run the echo-rust backend and send the corresponding notices to port `5004`.
...
When you add an input, you should see it being processed by the echo-rust backend as follows:
