...
Calculator is a customized DApp written in Python, which originally resembles the one provided by the sample [Echo DApp](https://github.com/cartesi/rollups-examples/tree/main/echo).

The documentation below is simply a copy of the one provided there and should be used to both run the original calculator code and as a basis for the documentation of any DApp created based on this calculator.
...
```shell
$ npx hardhat --network localhost calculator:addInput --input "0x63617274657369"
```
...
```shell
$ npx hardhat --network localhost calculator:getNotices --epoch 0
```
...
```shell
$ npx hardhat --network localhost calculator:addInput --input 'Hello there!'
```
