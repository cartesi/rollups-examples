...
This example shows how to build and interact with a minimalistic Cartesi Rollups application that runs an SQLite database internally. You can send any valid SQL command as input and if it produces results you get those back as a notice.
...
Then, build the back-end for the sqlite example:
...
In order to start the sqlite server, run the following commands in a dedicated terminal:
...
This will run the sqlite server on port `5003` and send the corresponding notices to port `5004`. The server will also automatically reload if there is a change in the source code, enabling fast development iterations.
...
When you add an input, you should see it being processed by the sqlite server as follows:
