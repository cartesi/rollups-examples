...
This example shows how to build and interact with a minimalistic Cartesi Rollups application that simply copies (or "echoes") each input received as a corresponding output notice.

This DApp's back-end is written in javascript with QuickJS.
...
First, go to a separate terminal window, switch to the `echo-js/contracts` directory, and run `yarn`:

```shell
$ cd echo-js/contracts/
$ yarn
```
...
The next step is to run the echo server in your machine. The application is written in javascript using QuickJS, so you need to have `qjs` installed.
