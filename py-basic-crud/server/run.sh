...
# Start the Cartesi HTTP-Dispatcher and the py-basic-crud-dapp.
# This script must run inside the cartesi machine

DAPP_PORT=5003
HTTP_DISPATCHER_PORT=5004

# Change dir to py-basic-crud-dapp root
cd /mnt/py-basic-crud-dapp

# Start py-basic-crud dapp
echo -n "Starting py-basic-crud-dapp: "
HTTP_DISPATCHER_URL="http://127.0.0.1:$HTTP_DISPATCHER_PORT" \
gunicorn --preload --workers 1 --bind 127.0.0.1:$DAPP_PORT py-basic-crud:app &

# Wait for the py-basic-crud dapp to start up
RETRY=0
...
do
    ...
    if [ "$RETRY" == "100" ]
    then
        echo "py-basic-crud dapp timed out"
        return 1
    fi
done
echo ""
